  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        <TotalCard icon={BarChart3} label="Partidas" value={totals.matches} tone="text-sky-300 bg-sky-400/10" />
        <TotalCard icon={Goal} label="Gols" value={totals.goals} tone="text-neon bg-neon/10" />
        <TotalCard icon={Square} label="Amarelos" value={totals.yellows} tone="text-amber-300 bg-amber-400/10" />
        <TotalCard icon={Square} label="Vermelhos" value={totals.reds} tone="text-red-300 bg-red-400/10" />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="card-arena p-4">
          ...
          {!stats.isLoading && scorers.length === 0 && (
            <p className="mt-6 text-center text-[10px] text-muted-foreground">Sem gols registrados.</p>
          )}
          {scorers.length > 0 && (
            <div className="mt-3 divide-y divide-white/[0.05]">
              {scorers.map((s, i) => (
                <div key={s.athlete_id} className="grid grid-cols-[28px_1fr_auto] items-center gap-2 py-2 text-xs">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-[9px] text-muted-foreground">{s.team_name ?? "—"}</p>
                  </div>
                  <span className="number-tabular font-display text-sm font-extrabold text-neon">{s.goals}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="card-arena p-4">
          ...
          {!stats.isLoading && cards.length === 0 && (
            <p className="mt-6 text-center text-[10px] text-muted-foreground">Sem cartões registrados.</p>
          )}
          {cards.length > 0 && (
            <div className="mt-3 divide-y divide-white/[0.05]">
              {cards.map((c) => (
                <div key={c.athlete_id} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 py-2 text-xs">
                  <div className="min-w-0">
                    <p className="truncate font-semibold">{c.athlete_name}</p>
                    <p className="truncate text-[9px] text-muted-foreground">{c.team_name ?? "—"}</p>
                  </div>
...
}: { icon: typeof BarChart3; label: string; value: number; tone: string }) {
  return (
    <article className="card-arena flex items-center gap-3 p-4">
      ...
      <div>
        <p className="font-display text-2xl font-extrabold leading-none tracking-[-0.05em]">{value}</p>
