...
export function MatchesPage({ championshipId }: { championshipId: string }) {
  return (
    <div className="space-y-4">
      ...
      <MatchesBoard championshipId={championshipId} />
    </div>
  );
}
...
function MatchesBoard({ championshipId }: { championshipId: string }) {
  ...
  const filters = useMemo<MatchFilters>(
    ...
    [status, teamId, round, date],
  );
  const matches = useMatches(championshipId, filters);
  ...
  return (
    <>
      <div className="grid gap-2 md:grid-cols-5">
        <Select
          value={status ?? "all"}
          onValueChange={(value) => setStatus(value as MatchFilters["status"])}
        >
          ...
          <SelectContent>
            ...
            {(["scheduled", "live", "finished", "postponed", "cancelled"] as MatchStatus[]).map(
              (value) => (
...
export function MatchPanel({
  ...
  const updateStatus = useSetMatchStatus(championshipId, match.id);
  ...
  const setStatus = async (status: MatchStatus) => {
    try {
      ...
      const reason = needsReason ? prompt("Informe o motivo desta alteração:") : undefined;
      if (needsReason && !reason?.trim()) return;
      if (status === "finished" && !confirm("Finalizar a partida e atualizar a classificação?"))
        return;
      if (
        match.status === "finished" &&
        status === "live" &&
        !confirm("Reabrir esta partida? A ação será auditada.")
      )
        return;
      await updateStatus.mutateAsync({ status, reason: reason ?? undefined });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          <Button
            ...
            disabled={
              !(["scheduled", "preparing", "postponed"] as MatchStatus[]).includes(match.status)
            }
          >
            Editar
          </Button>
          <Button
            ...
            onClick={() => setStatus(match.status === "finished" ? "live" : "scheduled")}
            disabled={match.status !== "postponed" && match.status !== "finished"}
          >
            <CalendarClock className="h-3 w-3" />{" "}
            {match.status === "finished" ? "Reabrir" : "Reagendar"}
          </Button>
          <Button
            size="sm"
            variant="outline"
            ...
            disabled={match.status !== "scheduled" && match.status !== "preparing"}
          >
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        <TeamScoreCard name={match.home_team?.name ?? "—"} score={match.home_score ?? 0} />
        ...
        <TeamScoreCard name={match.away_team?.name ?? "—"} score={match.away_score ?? 0} />
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button variant="outline" size="sm" asChild>
          <Link
            to="/championships/$id/matches/$matchId/report"
            params={{ id: championshipId, matchId: match.id }}
          >
            Abrir súmula
          </Link>
        ...
        {match.status === "live" && (
          <>
            <Button variant="outline" size="sm" onClick={() => setStatus("postponed")}>
              Adiar
            </Button>
...
function TeamScoreCard({ name, score }: { name: string; score: number }) {
  return (
    <div className="rounded-xl border border-white/[0.07] bg-black/20 p-3 text-center">
      ...
      <div className="number-tabular mt-2 font-display text-xl font-extrabold">{score}</div>
    </div>
  );
}
...
function EditMatchDialog({
  ...
  useEffect(() => {
    if (!open) return;
    setScheduledAt(
      match.scheduled_at ? new Date(match.scheduled_at).toISOString().slice(0, 16) : "",
    );
  ...
  const save = async () => {
    const changes: UpdateMatchInput = {
      scheduled_at: scheduledAt ? new Date(scheduledAt).toISOString() : null,
